...
trap 'log ERROR_ "Exiting with status $?"' EXIT
...
while true; do
    ...
    RESULT="$(curl -sS -X POST "http://supervisor/core/api/states/$ENTITY_ID" \
                       -H "Authorization: Bearer $SUPERVISOR_TOKEN" \
                       -H "Content-Type: application/json" \
                       -d '{"state":"unknown","attributes":{"'"${ENTITY_ID}"'-dummy":'"$(( $(date "+%M") % 2 ))"'}}')"
    RET="$?"
    if [ "$RET"   -ne 0 ]; then
    ...
    while true; do
        # beautysh: disable
        SQL_OUTPUT="$(sqlite3 -batch -bail "$DB" 2>&1  <<EOF
BEGIN IMMEDIATE;
UPDATE states
